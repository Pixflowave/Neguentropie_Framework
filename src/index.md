...
Plot.plot({
  ...
  marks: [
    ...
    // Ajouter une marque pour ajuster l'alignement du texte
    Plot.text(Néguanthropique, {
      x: (d) => d.depth === 0 ? 10 : -10, // Ajuster la position x
      y: (d) => d.y,
      text: (d) => d.name,
      textAnchor: (d) => d.depth === 0 ? 'start' : 'end',
      fontSize: 12,
      fill: 'black',
    })
  ]
